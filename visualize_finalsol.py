...
def generate_loci_colors(num_loci):
    '''
    Given a number of loci, generate a random set of colors (one color for each loci)
    '''
...
def kamada_kawai_viz(G, node_attr_df):
    ...
    nx.draw(G, node_color=node_attr_df.color, font_size=20, pos=nx.kamada_kawai_layout(G),with_labels = True, node_size=[d[k]*500 for k in d],edge_color="grey")
...
def circular_viz(G, node_attr_df):
    ...
    nx.draw(G, node_color=node_attr_df.color, font_size=50, pos=nx.circular_layout(G),with_labels = True, node_size=[d[k]*1000 for k in d],edge_color="grey")
